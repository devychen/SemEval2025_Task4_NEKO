...
def get_args_and_verify():
    ...
    # If specified, verify MIA data files exist
    ...
    # Add warning if MMLU file not provided
...
def compute_metrics(args):
    ...
    for split in ['forget', 'retain']:
        ...
        if len(files) == 0:
            print("Missing inference files, rerun script with inference first")
            sys.exit(1)
    ...
    if args.mia_data_path is not None:
        ...
        for dataset in ['member', 'nonmember']:
            ...
            if len(files) == 0:
                print("Missing mia files, rerun script with inference first")
                sys.exit(1)
            df_list = [pd.read_csv(f) for f in files]
            df = pd.concat(df_list, ignore_index=True)
        ...
        aggregate_scores_list.append(auc)
    ...
    results['aggregate-score'] = harmonic_mean(aggregate_scores_list)
